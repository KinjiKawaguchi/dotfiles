...
- **コメント/docstringの自己完結性**: 「このファイルだけを初めて開いた人」になりきって読み直す。以下は execution 中の自分（設計全体を作業記憶に持つ視点）が漏らしやすい兆候: (a) 呼び出し側の事情をそのファイルの docstring で語っている（呼ばれる側は自分の契約だけを書く）, (b) このセッション/設計会話でしか定義されていない固有名詞をファイル内で未定義のまま使っている, (c) 変更の経緯やユーザーとの質疑応答をほぼそのまま転記している（現在の実装意図ではなく履歴になっている）, (d) コード識別子（関数名・アルゴリズム名）がバッククォートで区切られず地の文の助詞に溶けて不自然な文になっている（例:「embed_text でベクトル化し」）, (e) 借用語のCS用語がそのプロジェクトで定着済みか未確認のまま何食わぬ顔で使われている
